...
1. Order statistics
   + np.ptp(): range of values (maximum - minimum) along an axis.
    (Deprecated since NumPy 1.24. Use np.max(a) - np.min(a) instead.)
...
NOTE: Deprecated since NumPy 1.24. Use np.max(a) - np.min(a) as a replacement.
'''
...
print(np.max(M1, axis=0) - np.min(M1, axis=0))

# [14  3  5 14]  (range of each column)

print(np.max(M1, axis=1) - np.min(M1, axis=1))

# [15 13 15]  (range of each row)
...
Equivalent to np.percentile(a, q * 100). q must be in the range [0.0, 1.0].
'''
...
print(np.median(M1, axis=0, keepdims=True))
...
# Mean over all elements (no axis argument)
print(np.mean(M1))
...
print("Cov(A,B) :", cov_AB[0, 1].round(4))
print("Var(A)   :", cov_AB[0, 0].round(4))
print("Var(B)   :", cov_AB[1, 1].round(4))
